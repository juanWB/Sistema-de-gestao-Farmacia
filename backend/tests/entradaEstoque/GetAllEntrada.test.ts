...
describe("EntradaEstoqueController - GetAll", () => {
  ...
  let produto_id: number | undefined = undefined;
    beforeAll(async () => {
      const fonercedorId = await serverTest.post("/fornecedores").send({
        nome: "Atacamax",
      ...
      const response = await serverTest.post("/produtos").send({
        nome: "Sabonete",
        preco: "1.99",
        validade: "2025-01-01",
        quantidade: "100",
  ...
  it("Busca todas entradas no estoque", async () => {
    const res1 = await serverTest.post("/entradas").send({
      produto_id,
      quantidade: 2,
      entrada_data: "2000-06-17",
    }).set({Authorization: `Bearer ${accessToken}`});

    const res2 = await serverTest.get("/entradas").set({Authorization: `Bearer ${accessToken}`});
